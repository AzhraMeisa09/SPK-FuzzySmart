...
<div class="space-y-6 pb-12">
    ...
    <div class="card p-6 flex flex-col sm:flex-row justify-between items-center gap-6 shadow-xl border-none relative overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto relative no-print">
            <form action="{{ route('admin.hasil_evaluasi') }}" method="GET" class="w-full sm:w-auto flex items-center gap-2">
                <span class="text-xs font-bold text-var(--text-3) whitespace-nowrap">Pilih Periode:</span>
                <select name="periode_id" onchange="this.form.submit()" class="form-select bg-var(--bg) border-var(--border) rounded-xl text-[13px] font-bold h-[42px] min-w-[180px]" style="padding-left: 16px; padding-right: 36px; cursor: pointer;">
            ...
            <button onclick="window.print()" class="btn btn-green shadow-lg shadow-green-100 px-6 py-2.5 rounded-xl flex items-center justify-center gap-2 font-bold text-sm w-full sm:w-auto">
    ...
    @if(!$isFinalized)
        {{-- TAMPILKAN PROGRES PENGISIAN JIKA BELUM FINALISASI --}}
        <div class="card p-8 border-none shadow-xl">
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 pb-4 border-b border-gray-100">
                <div>
                    <h3 class="text-lg font-bold text-var(--text-1) tracking-tight">Monitoring Progres Pengisian Nilai</h3>
                    <p class="text-xs text-var(--text-3) font-medium mt-1">Pemantauan real-time kelengkapan data pengisian oleh guru sebelum finalisasi.</p>
                </div>
                <div class="flex items-center gap-2">
                    <span class="px-3 py-1 rounded-lg bg-amber-50 text-amber-600 border border-amber-100 text-[10px] font-bold">
                        Dalam Proses Penginputan
                    </span>
                    <a href="{{ route('admin.periode.index') }}" class="btn btn-green text-xs px-4 py-2 rounded-xl font-bold flex items-center gap-1.5 shadow-sm">
                        Kelola Periode & Finalisasi
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                @forelse($progresPengisian as $prog)
                    <div class="p-6 rounded-2xl bg-var(--bg)/50 border border-var(--border)/50 hover:border-var(--accent)/30 hover:bg-white transition-all shadow-sm group font-sans">
                        <div class="flex items-center justify-between mb-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 rounded-xl bg-var(--accent-lt) text-var(--accent) flex items-center justify-center font-bold text-sm shadow-sm border border-var(--accent)/10">
                                    {{ substr($prog->kelas->nama_kelas, 0, 2) }}
                                </div>
                                <div>
                                    <h4 class="text-sm font-bold text-var(--text-1) group-hover:text-var(--accent) transition-colors leading-tight">Kelas {{ $prog->kelas->nama_kelas }}</h4>
                                    <p class="text-[10px] text-var(--text-3) font-semibold mt-0.5">{{ $prog->total_siswa }} Siswa Terdaftar</p>
                                </div>
                            </div>
                            <span class="text-lg font-black {{ $prog->persen >= 100 ? 'text-green-600' : ($prog->persen >= 50 ? 'text-amber-500' : 'text-red-500') }}">
                                {{ $prog->persen }}%
                            </span>
                        </div>

                        <div class="h-2.5 bg-gray-100 rounded-full overflow-hidden p-0.5 border border-gray-200/50 mb-4">
                            <div class="h-full rounded-full transition-all duration-1000 {{ $prog->persen >= 100 ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.3)]' : ($prog->persen >= 50 ? 'bg-amber-400' : 'bg-red-400') }}"
                                 style="width: {{ $prog->persen }}%"></div>
                        </div>

                        <div class="flex items-center justify-between text-[11px] font-bold text-var(--text-3)">
                            <div class="flex items-center gap-1.5">
                                <span class="w-2 h-2 rounded-full bg-green-500"></span>
                                <span>{{ $prog->total_final }} Final</span>
                            </div>
                            <div class="flex items-center gap-1.5">
                                <span class="w-2 h-2 rounded-full bg-amber-400"></span>
                                <span>{{ $prog->total_draft }} Draft</span>
                            </div>
                            <span>Total Terisi: {{ $prog->total_isi }}/{{ $prog->total_harus }}</span>
                        </div>
                    </div>
                @empty
                    <div class="col-span-2 py-10 text-center text-var(--text-3) text-xs font-semibold">
                        Tidak ada kelas yang terdaftar pada periode ini.
                    </div>
                @endforelse
            </div>
            
            <div class="mt-8 p-4 rounded-xl bg-blue-50 border border-blue-100 text-xs font-semibold text-blue-700 flex items-center gap-2">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                <span>Sistem akan melakukan perhitungan perangkingan Fuzzy SMART secara otomatis setelah tombol <strong>Finalisasi Periode</strong> ditekan oleh Admin di menu Kelola Periode.</span>
            </div>
        </div>
    @else
                                        ...
                                        <tr class="hover:bg-white transition-colors group">
                                            ...
                                            <td class="text-center">
                                                @if($item->kategori_akhir == 'BSB')
                                                    <span class="px-3 py-1.5 bg-green-50 text-green-700 border border-green-100 rounded-lg text-[10px] font-bold uppercase">Berkembang Sangat Baik (BSB)</span>
                                                @elseif($item->kategori_akhir == 'BSH')
                                                    <span class="px-3 py-1.5 bg-amber-50 text-amber-700 border border-amber-100 rounded-lg text-[10px] font-bold uppercase">Berkembang Sesuai Harapan (BSH)</span>
                                                @else
                                                    <span class="px-3 py-1.5 bg-red-50 text-red-700 border border-red-100 rounded-lg text-[10px] font-bold uppercase">Mulai Berkembang (MB)</span>
                                                @endif
                                            </td>
