@php
    ...
    $periodeAktif = \App\Models\PeriodePenilaian::where('is_aktif', true)->with('tahunAjaran')->first();
@endphp

<header class="h-14 bg-white flex items-center justify-between px-4 z-30 sticky top-0" style="border-bottom: 1px solid var(--border);">
    <!-- Left -->
    ...
    <div class="flex items-center gap-2">
        <div class="hidden sm:block">
             <div class="text-xs font-semibold text-[#84934A] bg-[#84934A]/10 px-3 py-1 rounded-full border border-[#84934A]/20">
                 {{ $periodeAktif ? $periodeAktif->nama_periode : 'Tidak Ada Periode Aktif' }}
             </div>
        </div>
        ...
        <div class="relative" x-data="{ open: false }">
            <button @click="open = !open" class="flex items-center gap-2 pl-2 pr-1 py-1 rounded-lg hover:bg-gray-100 transition-colors">
                <div class="text-right hidden sm:block">
                    <p id="navUserName" class="text-xs font-semibold text-gray-800 leading-tight">{{ auth()->user()->nama_lengkap ?? 'User' }}</p>
                    <p id="navUserRole" class="text-[10px] text-gray-400 uppercase tracking-tighter">{{ str_replace('_', ' ', auth()->user()->role ?? 'Role') }}</p>
                </div>
                <div id="navUserInitial" class="w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold overflow-hidden" style="background: #84934A;">
