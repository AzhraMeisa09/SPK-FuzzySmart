...
<div class="space-y-6 fade-in pb-12">
    ...
    @if(count($anak) > 1)
        <div class="flex flex-wrap items-center gap-3 no-print">
            @foreach($anak as $a)
                <a href="{{ route('wali.laporan', ['siswa_id' => $a->id_siswa]) }}" 
    ...
    @if(!$evaluasi)
        <div class="card p-20 text-center">
            ...
            <p class="text-xs mt-2 max-w-sm mx-auto" style="color: var(--text-3);">
                Pratinjau laporan akhir untuk {{ $siswa->name }} belum dapat ditampilkan karena periode penilaian belum difinalisasi oleh pihak sekolah.
            </p>
            <div class="mt-8">
                    ...
                    <div class="flex items-center gap-6">
                        <div class="w-20 h-20 rounded-3xl bg-white border-4 border-white shadow-xl overflow-hidden flex-shrink-0 ring-1 ring-gray-100">
                            @if($siswa->foto)
                                <img src="{{ asset('storage/' . $siswa->foto) }}" class="w-full h-full object-cover" alt="{{ $siswa->name }}">
                            @else
                                <div class="w-full h-full bg-var(--accent-lt) flex items-center justify-center text-3xl font-black text-var(--accent)">
            ...
            <div class="p-10 md:p-14 space-y-12">
                ...
                <section>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="w-8 h-8 rounded-lg text-white text-sm font-semibold flex items-center justify-center flex-shrink-0" style="background: var(--accent);">I</span>
                        <h4 class="text-base font-semibold" style="color: var(--text-1);">Capaian Per Aspek Perkembangan</h4>
                    </div>
                ...
                <section>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="w-8 h-8 rounded-lg text-white text-sm font-semibold flex items-center justify-center flex-shrink-0" style="background: var(--accent);">II</span>
                        <h4 class="text-base font-semibold" style="color: var(--text-1);">Daftar Capaian Indikator</h4>
                    </div>
                            ...
                            <tbody>
                                @foreach($evaluasi->detail as $d)
                                    ...
                                    <tr class="hover:bg-blue-50/20 transition-colors">
                                        <td class="text-center"><span class="badge badge-blue font-mono text-[9px]">{{ $d->subkriteria->id_subkriteria }}</span></td>
                                        <td class="py-5">
                                            <p class="text-sm font-bold text-gray-800 leading-snug">{{ $d->subkriteria->nama_subkriteria }}</p>
                                            @if($rubrikText)
                                                <p class="text-[11px] text-gray-500 italic mt-1.5 leading-relaxed">&ldquo;{{ $rubrikText }}&rdquo;</p>
                                            @endif
                                            <p class="text-[9px] font-bold text-gray-400 uppercase tracking-widest mt-1">{{ $d->subkriteria->kriteria->nama_kriteria }}</p>
                                        </td>
                ...
                <section>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="w-8 h-8 rounded-lg text-white text-sm font-semibold flex items-center justify-center flex-shrink-0" style="background: var(--accent);">III</span>
                        <h4 class="text-base font-semibold" style="color: var(--text-1);">Rekomendasi Strategis per Aspek</h4>
                    </div>
                    <div class="space-y-6">
                        ...
                        @foreach($details as $kriteriaName => $items)
                            ...
                            @if($rekList->isNotEmpty())
                                ...
                                <div class="p-8 rounded-[2rem] bg-gray-50 border border-gray-100">
                                    <h5 class="text-xs font-black text-var(--accent) uppercase tracking-[0.2em] mb-6 flex items-center gap-3">
                ...
                <section>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="w-8 h-8 rounded-lg text-white text-sm font-semibold flex items-center justify-center flex-shrink-0" style="background: var(--accent);">IV</span>
                        <h4 class="text-base font-semibold" style="color: var(--text-1);">Portofolio & Dokumentasi</h4>
                    </div>
                    <div class="card overflow-hidden border-gray-100">
                        <table class="tbl">
                            <thead>
                ...
                <section>
                    <div class="flex items-center gap-3 mb-6">
                        <span class="w-8 h-8 rounded-lg text-white text-sm font-semibold flex items-center justify-center flex-shrink-0" style="background: var(--accent);">V</span>
                        <h4 class="text-base font-semibold" style="color: var(--text-1);">Kesimpulan Guru</h4>
                    </div>
